...
describe('OTPublisher', () => {
	...
	beforeEach(() => {
		publisher = jasmine.createSpyObj('publisher', ['on', 'once', 'off', 'destroy']);
	...
	describe('with session prop', () => {
		...
		beforeEach(() => {
			session = jasmine.createSpyObj('session', ['once', 'off', 'publish', 'unpublish']);
		});
		...
		describe('connected session', () => {
			...
			it('should listen for streamCreated publisher event', () => {
				expect(publisher.on).toHaveBeenCalledWith('streamCreated', jasmine.any(Function));
			});
			...
			it('should not listen for sessionConnected event', () => {
				expect(session.once).not.toHaveBeenCalledWith('sessionConnected', jasmine.any(Function));
			});

			it('should have a valid publisherId', () => {
				const { publisherId } = wrapper.instance();
				const uuidV4Regex = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
				expect(uuidV4Regex.test(publisherId)).toBe(true);
			});
		});
		...
		describe('disconnected session', () => {
			...
			it('should listen for sessionConnected event', () => {
				expect(session.once).toHaveBeenCalledWith('sessionConnected', jasmine.any(Function));
			});
			...
			it('should stop listening for sessionConnected when unmounting', () => {
				const sessionConnectedHandler = session.once.calls.first().args[1];
				wrapper.unmount();
				expect(session.off).toHaveBeenCalledWith('sessionConnected', sessionConnectedHandler);
			});

			it('should stop listening for streamCreated when unmounting', () => {
				const streamCreatedHandler = publisher.on.calls.first().args[1];
				wrapper.unmount();
				expect(publisher.off).toHaveBeenCalledWith('streamCreated', streamCreatedHandler);
			});
